...
export function MedicationControlModal({
  isOpen,
  onClose,
  medication,
  onSave,
  ...
  const getValidationMessage = () => {
    if (!hasAllTests && !hasAllPhases)
      return "Complete all tests & activate all phases";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
      <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 w-full max-w-md rounded-2xl shadow-xl overflow-hidden text-sm transition-colors">
        ...
        <div className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-500 mb-2">
              Active Phases (Status Indicators)
            </label>
        ...
        <div className="px-6 py-4 bg-slate-50 dark:bg-slate-900/50 border-t border-slate-100 dark:border-slate-700 flex justify-end gap-2">
          ...
          <button
            onClick={handleSave}
            className="px-4 py-2 text-xs font-bold bg-blue-600 text-white rounded-xl hover:bg-blue-700 transition cursor-pointer"
          >
            Save Updates
          </button>
        </div>
